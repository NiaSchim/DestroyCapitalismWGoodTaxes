...
class Economy:
    ...
    def perform_nianomics(self):
        ...
        self.treasury += total_donation_rate  # Add the total donation rate to the treasury instead of subtracting it
    ...
    def balance_treasury(self):
        ...
        if abs(self.treasury - average_wealth) > 1e-6:  # If the treasury and average wealth are not equal
            for agent in self.agents:
                wealth_adjustment = (average_wealth - agent.wealth) / (threemonths / 3)  # Calculate the amount to be adjusted in each tick
                agent.update_wealth(wealth_adjustment)  # Adjust the agent's wealth in each tick
            treasury_adjustment = (average_wealth - self.treasury) / (threemonths / 3)  # Calculate the amount to be adjusted in the treasury in each tick
            self.treasury += treasury_adjustment  # Adjust the treasury in each tick
...
if __name__ == "__main__":
    ...
    # Get the best point from the optimizer
    ...
    personalities = ['loss_aversion', 'overconfidence', 'altruism', 'envy', 'default']
    agents = [Agent(i, initial_wealth_per_agent, random.choice(personalities)) for i in range(num_agents)]  # Create a list of 11192 agents with initial wealth of approximately $1107.19 and random personalities
    economy = Economy(agents, tax_algorithm(best_point))  # Create an economy with the agents and a tax algorithm

    # Run the economy for a certain amount of time
    start_time = time.time()
    tax_interval = threemonths / 3 / 24  # Taxes are performed every 24th of a third of a threemonth
    next_tax_time = start_time + tax_interval
    while time.time() - start_time < threemonths:  # Run for X seconds times some number
        current_time = time.time()
        if current_time >= next_tax_time:
            economy.perform_taxes()
            next_tax_time += tax_interval
        economy.perform_nianomics()
        economy.balance_treasury()
