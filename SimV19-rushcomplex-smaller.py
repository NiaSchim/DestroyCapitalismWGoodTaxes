...
def tax_algorithm(params):
    ...
    def calculate_tax(wealth):
        if wealth >= translation_ammount:
            return (wealth - translation_ammount)**((higher_power)+1)
        else:
            return ((0-(((wealth+translation_ammount)+(lower_power/math.sqrt(wealth+translation_ammount)))/1))-lower_power)
...
class CustomOptimizer:
    ...
    def explore(self):
        while self.consecutive_unsuccessful_runs < self.max_consecutive_failures:
            ...
            if self.visit_count[tuple(new_point)] >= 4:
