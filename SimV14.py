...
threemonths = 3 * 60
onefourthsample = 100  # replace with the actual number
...
class Economy:
    def __init__(self, agents, tax_scheme, start_point=None):
        self.agents = agents
        self.transactions = []
...
def objective_function(point, memory):
    ...
    economy = Economy(agents, tax_algorithm(point))
    ...
    distances = distance.pdist(wealthdays, 'euclidean')
    shortstraw = np.min(distances)
    yardstick = np.linalg.norm(economy.start_point - np.mean(wealthdays, axis=0))
...
if __name__ == "__main__":
    ...
    personalities = ['loss_aversion', 'overconfidence', 'altruism', 'envy', 'default']
    agents = [Agent(i, initial_wealth_per_agent, random.choice(personalities)) for i in range(num_agents)]
    economy = Economy(agents, tax_algorithm(best_point))
    ...
    average_distribution = np.mean(economy.wealth_distribution_over_time, axis=0)
    deviation_from_line = average_distribution - np.mean(average_distribution)
    fairityscore = 100 - np.mean(np.abs(deviation_from_line))
    score = fairityscore

    print("Wealth Dynamism Grade: {:.2f} out of 100%".format(dynamism * 100))
    print("Wealth Pattern Avoidance Grade: {:.2f} out of 100%".format(fairityscore))
    print("GPA: {:.2f}".format((dynamism + fairityscore) / 2))
